...
pub fn generate_test_repo(commits: usize, files_per_commit: usize) -> (TempDir, PathBuf) {
    ...
    for i in 0..commits {
        for j in 0..files_per_commit {
            let filename = format!("src/file_{}_{}.rs", i, j);
            let workdir = repo.workdir().unwrap();
            let file_path = workdir.join(&filename);
            
            std::fs::create_dir_all(file_path.parent().unwrap()).ok();
            std::fs::write(&file_path, format!("// File {} content with some lines\nfn test() {{\n    let x = {};\n}}\n", filename, i * j)).ok();
            ...
            let message = format!("Commit {} with file {}", i, j);
...
pub fn generate_git_log_output(commits: usize, files_per_commit: usize) -> String {
    ...
    for i in 0..commits {
        ...
        let message = format!("Commit {} with {} files", i, files_per_commit);
        
        output.push_str(&format!("commit {}|{}|{}\n", hash, timestamp, message));
        
        for j in 0..files_per_commit {
            let added = 10 + (i * j % 100) as usize;
            let deleted = 5 + (i * j % 50) as usize;
            let filename = format!("src/file_{}_{}.rs", i, j);
